OwnersController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('owners', this.getCriteria(), sort);
    },
...
{itemTye: {$regex: search, $options: 'i'}},
{primaryOwner: {$regex: search, $options: 'i'}},
{coOwners: {$regex: search, $options: 'i'}},
...
{contributers: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Owners.find(this.getCriteria(), sort);

        return this.render('ownersIndex', {
            data: {
...
itemTye: t.find('#itemTye').value,
primaryOwner: t.find('#primaryOwner').value,
coOwners: t.find('#coOwners').value,
...
contributers: t.find('#contributers').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Owners.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Owners");
                Router.go('ownersView', {_id: _id});
            });
        }
        return this.render('ownersInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Owners.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Owners");
            });
            Router.go('ownersView', {_id: _id});
        }
        return this.render('ownersUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Owners.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Owners");
        });
    },
    _loadModel: function(_id) {
        return Owners.findOne(_id);
    },
});