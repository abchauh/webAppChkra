SuggestsController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('suggests', this.getCriteria(), sort);
    },
...
{itemID: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Suggests.find(this.getCriteria(), sort);

        return this.render('suggestsIndex', {
            data: {
    ...
    view: function() {
        return this.render('suggestsView', {
            data: {
...
itemID: t.find('#itemID').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Suggests.insert(doc, function(err, _id) {
                ...
                MeteorisFlash.set('success', "Success Inserting Suggests");
                Router.go('suggestsView', {_id: _id});
            });
        }
        return this.render('suggestsInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Suggests.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Suggests");
            });
            Router.go('suggestsView', {_id: _id});
        }
        return this.render('suggestsUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Suggests.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Suggests");
        });
    },
    _loadModel: function(_id) {
        return Suggests.findOne(_id);
    },
});