BlocksController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('blocks', this.getCriteria(), sort);
    },
...
{tags: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Blocks.find(this.getCriteria(), sort);

        return this.render('blocksIndex', {
            data: {
...
tags: t.find('#tags').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Blocks.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Blocks");
                Router.go('blocksView', {_id: _id});
            });
        }
        return this.render('blocksInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Blocks.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Blocks");
            });
            Router.go('blocksView', {_id: _id});
        }
        return this.render('blocksUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Blocks.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Blocks");
        });
    },
    _loadModel: function(_id) {
        return Blocks.findOne(_id);
    },
});