MeteorSkillsController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('meteorSkills', this.getCriteria(), sort);
    },
...
{level: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = MeteorSkills.find(this.getCriteria(), sort);

        return this.render('meteorSkillsIndex', {
            data: {
    ...
    view: function() {
        return this.render('meteorSkillsView', {
            data: {
...
level: t.find('#level').value? Number(t.find('#level').value):null,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            MeteorSkills.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting MeteorSkills");
                Router.go('meteorSkillsView', {_id: _id});
            });
        }
        return this.render('meteorSkillsInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            MeteorSkills.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating MeteorSkills");
            });
            Router.go('meteorSkillsView', {_id: _id});
        }
        return this.render('meteorSkillsUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        MeteorSkills.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing MeteorSkills");
        });
    },
    _loadModel: function(_id) {
        return MeteorSkills.findOne(_id);
    },
});