NewsFeedsLikedController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('newsFeedsLiked', this.getCriteria(), sort);
    },
...
{order: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = NewsFeedsLiked.find(this.getCriteria(), sort);

        return this.render('newsFeedsLikedIndex', {
            data: {
    ...
    view: function() {
        return this.render('newsFeedsLikedView', {
            data: {
    ...
    _getDoc: function(t) {
        return {
            itemId: t.find('#itemId').value,
order: t.find('#order').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            NewsFeedsLiked.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting NewsFeedsLiked");
                Router.go('newsFeedsLikedView', {_id: _id});
            });
        }
        return this.render('newsFeedsLikedInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            NewsFeedsLiked.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating NewsFeedsLiked");
            });
            Router.go('newsFeedsLikedView', {_id: _id});
        }
        return this.render('newsFeedsLikedUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        NewsFeedsLiked.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing NewsFeedsLiked");
        });
    },
    _loadModel: function(_id) {
        return NewsFeedsLiked.findOne(_id);
    },
});