FeedsController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('feeds', this.getCriteria(), sort);
    },
...
{order: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Feeds.find(this.getCriteria(), sort);

        return this.render('feedsIndex', {
            data: {
    ...
    _getDoc: function(t) {
        return {
            itemType: t.find('#itemType').value,
itemId: t.find('#itemId').value,
status: t.find('#status').value,
order: t.find('#order').value? Number(t.find('#order').value):null,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Feeds.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Feeds");
                Router.go('feedsView', {_id: _id});
            });
        }
        return this.render('feedsInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Feeds.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Feeds");
            });
            Router.go('feedsView', {_id: _id});
        }
        return this.render('feedsUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Feeds.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Feeds");
        });
    },
