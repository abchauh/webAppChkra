UpdateArticleController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('updateArticle', this.getCriteria(), sort);
    },
...
{type: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = UpdateArticle.find(this.getCriteria(), sort);

        return this.render('updateArticleIndex', {
            data: {
    ...
    view: function() {
        return this.render('updateArticleView', {
            data: {
...
type: t.find('#type').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            UpdateArticle.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting UpdateArticle");
                Router.go('updateArticleView', {_id: _id});
            });
        }
        return this.render('updateArticleInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            UpdateArticle.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating UpdateArticle");
            });
            Router.go('updateArticleView', {_id: _id});
        }
        return this.render('updateArticleUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        UpdateArticle.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing UpdateArticle");
        });
    },
    _loadModel: function(_id) {
        return UpdateArticle.findOne(_id);
    },
});