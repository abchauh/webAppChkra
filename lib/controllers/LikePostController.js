LikePostController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('likePost', this.getCriteria(), sort);
    },
...
{order: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = LikePost.find(this.getCriteria(), sort);

        return this.render('likePostIndex', {
            data: {
    ...
    view: function() {
        return this.render('likePostView', {
            data: {
...
order: t.find('#order').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            LikePost.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting LikePost");
                Router.go('likePostView', {_id: _id});
            });
        }
        return this.render('likePostInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            LikePost.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating LikePost");
            });
            Router.go('likePostView', {_id: _id});
        }
        return this.render('likePostUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        LikePost.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing LikePost");
        });
    },
    _loadModel: function(_id) {
        return LikePost.findOne(_id);
    },
});