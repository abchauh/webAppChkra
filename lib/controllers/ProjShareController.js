LikeProjController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('likeProj', this.getCriteria(), sort);
    },
...
{order: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = LikeProj.find(this.getCriteria(), sort);

        return this.render('likeProjIndex', {
            data: {
    ...
    view: function() {
        return this.render('likeProjView', {
            data: {
...
order: t.find('#order').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            LikeProj.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting LikeProj");
                Router.go('likeProjView', {_id: _id});
            });
        }
        return this.render('likeProjInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            LikeProj.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating LikeProj");
            });
            Router.go('likeProjView', {_id: _id});
        }
        return this.render('likeProjUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        LikeProj.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing LikeProj");
        });
    },
    _loadModel: function(_id) {
        return LikeProj.findOne(_id);
    },
});