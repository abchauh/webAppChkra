PromotedController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('promoted', this.getCriteria(), sort);
    },
...
{counter: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Promoted.find(this.getCriteria(), sort);

        return this.render('promotedIndex', {
            data: {
    ...
    view: function() {
        return this.render('promotedView', {
            data: {
...
counter: t.find('#counter').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Promoted.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Promoted");
                Router.go('promotedView', {_id: _id});
            });
        }
        return this.render('promotedInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Promoted.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Promoted");
            });
            Router.go('promotedView', {_id: _id});
        }
        return this.render('promotedUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Promoted.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Promoted");
        });
    },
    _loadModel: function(_id) {
        return Promoted.findOne(_id);
    },
});